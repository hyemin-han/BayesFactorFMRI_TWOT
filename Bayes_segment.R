...
# read file list
data<-read.csv(list)
data2 <- read.csv(list2)
filecount <-nrow(data)
filecount2 <- nrow(data2)
...
AllImg2 = array(0.0,c(X,Y,Z, filecount2))
...
# second group
for (i in 1:filecount2)  {
  filename2 <- toString(data2[i,1])
  # Read the current nifti
  Img2 <- readNIfTI(filename2)
  
  # Extract current image data
  ImgData2 = oro.nifti::img_data(Img2)
  
  # copy to the AllImg
  for (x in 1:X){
    for (y in 1:Y){
      for (z in 1:Z){
        AllImg2[x,y,z,i] <- ImgData2[x,y,z]
      }
    }
  }
}
